...
The environment declares `mattermost/enterprise` in `repositoryDependencies` for GitHub token scope. Environment builds only check out this repo; the install hook clones enterprise to `$HOME/enterprise` when the token can access it, and continues with open-source modules when it cannot.
