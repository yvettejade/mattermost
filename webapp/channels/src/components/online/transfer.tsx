...
import {findAccount, formatAudFromCents, parseAmountCents, submitOwnTransfer, transferableAccounts} from './store';
import type {AccountId, EverydayMoneyState, TransferError} from './types';
...
export default function Transfer({state, onStateChange}: Props) {
    ...
    const [toAccountId, setToAccountId] = useState<AccountId>(eligible[1]?.id ?? 'savings');
    ...
    const handleFromChange = useCallback((event: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        resetStatus();
        if (next === toAccountId) {
            const other = eligible.find((account) => account.id !== next);
            if (other) {
                setToAccountId(other.id);
            }
        }
    }, [eligible, resetStatus, toAccountId]);
    ...
    const handleConfirm = useCallback(() => {
        const result = submitOwnTransfer(fromAccountId, toAccountId, amount);
        if (!result.ok) {
            setSuccess(null);
            setReviewing(false);
        ...
        const toAccount = findAccount(result.state.accounts, toAccountId);
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    {eligible.filter((account) => account.id !== fromAccountId).map((account) => (
                        <option
                            key={account.id}
                            value={account.id}
                        >
