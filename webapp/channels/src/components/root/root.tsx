...
import {HOME_LOANS_URL_SUFFIX, HOME_URL_SUFFIX, PageLoadContext, RATES_URL_SUFFIX, SCHEDULED_POST_URL_SUFFIX} from 'utils/constants';
...
export function doesRouteBelongToTeamControllerRoutes(pathname: RouteComponentProps['location']['pathname']): boolean {
    ...
    const TEAM_CONTROLLER_PATH_PATTERN = new RegExp(`^/(?!admin_console)([a-z0-9\\-_]+)/(channels|messages|threads|recaps|drafts|integrations|emoji|${SCHEDULED_POST_URL_SUFFIX}|${HOME_LOANS_URL_SUFFIX}|${HOME_URL_SUFFIX}|${RATES_URL_SUFFIX})(/.*)?$`);
