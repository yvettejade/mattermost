...
func TestLocalGrokFallbackHelpAndIntegrations(t *testing.T) {
	...
	jira := localGrokFallback(model.GrokIntentJira, "YJIRA-24", grokWorkspaceContext{})
...
func TestLocalGrokFallback(t *testing.T) {
	t.Setenv("YvetteGrokAPI", "")
	t.Setenv("MM_GROKAGENTSETTINGS_APIKEY", "")

	reply := localGrokFallback(model.GrokIntentSummarize, "summarize", grokWorkspaceContext{
		ChannelName: "Search",
