...
Users stay in Mattermost: a **Grok** chat modal opened from the left sidebar immediately above Invite Members, plus `@yvette-grok` mentions, DMs to the bot, and `/yvette` slash commands.

The sidebar button and modal are a plugin webapp (`registerLeftSidebarAboveInviteMembersComponent` + `registerRootComponent`). Mattermost core exposes the `LeftSidebarAboveInviteMembers` pluggable; this plugin does not replace official `mattermost-ai`.
...
A **Grok** button sits in the Direct Messages category immediately above Invite Members. It opens a chat modal (`registerRootComponent`) that calls:

- `GET /plugins/com.yvette.grok-agent/api/v1/status`
- `POST /plugins/com.yvette.grok-agent/api/v1/query` with `{text, channel_id, root_id?, team_id?, intent?}`

Requests require a Mattermost session (`Mattermost-User-Id`). History is limited to channels the requester can access. The modal is read/reply only (no `CreatePost`); `/yvette schedule` still opens the in-channel proposal dialog.
